...
import { CONTINENTS, RESULT_LIMIT, TYPE_ORDER } from "./filters";
import { ToggleChip, TypeChip } from "./chips";

// The Airports pane — a Library TAB, not a router route. It is reached through
// `LibraryPage`'s tab strip and appears nowhere in `routeTree`.
//
// The table and the map ask the same question through one server-side WHERE
// builder (`airportFilter`), which is what guarantees they show the same set
// once anything is typed. The map's cap is far higher: it plots everything that
// matches while the table lists the top `RESULT_LIMIT`.
