...
/**
 * Where a source may run, and why that is a property of the source rather than
 * a deployment choice.
 *
 * - `worker` — the service authenticates the CREDENTIAL. seats.aero wants its
 *   key and does not care that Cloudflare made the request, so the Worker calls
 *   it directly and a search needs no laptop awake.
 * - `local` — the service judges the CLIENT, or its posture against a datacenter
 *   IP has never been measured. It runs from `packages/local-sources` on a
 *   residential connection and POSTs to `/api/ingest/*`.
 *
 * **This field records evidence, not preference.** Setting it to `worker`
 * without having measured it is how a source ends up quietly returning nothing
 * in production, which is indistinguishable from "there is no award space" —
 * the one failure this app is built to prevent.
 */
export type SourceRuntime = "worker" | "local";

/**
 * Identity and placement. Every source has one of these, and the registry is
...
/**
 * A source the **generic** runner can drive: plan the window into tasks, run
 * each one, hand the results to ingest.
 *
 * Not every source is one. seats.aero is a `SourceDescriptor` only, because the
 * Worker drives it through a specialised runner (`workers/api/src/searchRun.ts`)
 * that streams each HTTP call to the browser as it lands, meters a per-request
 ...
 * accounting into this interface — making every future source carry seats.aero's
 * shape. The split is by **who drives the source**, and it is deliberate.
 *
 * If you are adding a source, this is almost certainly what you implement.
 */
