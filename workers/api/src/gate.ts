 ...
 *   as "no gate", the posture `INGEST_TOKEN` takes — would mean one forgotten
 *   `wrangler secret put` silently publishes the whole app, and a security
 *   control whose failure mode is silence is not a security control.
 */
 ...
 * and the length of the real secret stays hidden. Exported because
 * `ingest.ts` checks the *same* `INGEST_TOKEN` this file's `gate` checks, and
 * two comparisons of one secret that disagree about their rigour is a hole with
 ...
 *
 * The local source runner is the one exception, and it is not a hole: it proves
 * itself with `INGEST_TOKEN`, a secret of exactly the same standing as this
 * one, on every POST it makes (`packages/local-sources`). Making it
 * carry a *browser* session instead would mean a headless local process
 * performing a login it has no UI for, to obtain a weaker credential than the
...
export async function gate(c: Context<{ Bindings: Env; Variables: Vars }>, next: Next) {
  ...
  const ingestToken = c.env.INGEST_TOKEN;
  const presentedIngestToken = c.req.header("X-Ingest-Token");
  if (presentedIngestToken) {
    if (ingestToken && (await secretsMatch(presentedIngestToken, ingestToken))) {
      return next();
    }
    // A caller presenting an ingest token is the local source runner, not a
    // browser, and telling it `locked` would send its operator hunting for a
    // password it was never going to type. Name the credential that failed.
