...
//
// **This is the one data source the Cloudflare Worker calls directly** — the
// only registered source declaring `runtime: "worker"` (docs/SOURCES.md). It is
// a keyed vendor API that authenticates the *key*, not the client, and is
// therefore indifferent to where the request comes from. A source that has not
// been shown to hold that property declares `runtime: "local"` instead and is
// run by `npm run gather`. Hence this file has two halves: pure parsing/normalizing
// below, and the chunk-and-paginate loop at the bottom that `workers/api` drives
// when a human presses Search on a tracked route.
//
...
// 5-15s — the local sources answer that for free.
//
...
// object, which is why it is a click (`workers/api/src/enrich.ts`) and never
...
// (`workers/api/src/searchRun.ts`); see docs/SOURCES.md.
...
// `workers/api/src/enrich.ts` and never by the search path.
//
  ...
  if (Array.isArray(raw) && raw.length) {
    ...
    return ordered.map((s) => {
      ...
      return {
        ...
        // Flight numbers arrive carrier-prefixed ("JX11"), as PointsYeah's do.
        // `flightLabel` in the SPA already strips a duplicated prefix, so keeping
        // the raw value and deriving the carrier from it matches that source
