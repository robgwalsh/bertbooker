...
/**
 * The Library's seats.aero pane: which city pairs each program's award
 * inventory is actually monitored on.
 *
 * Everything here reads a graph cached in D1. `SourceBar` is the only thing that
 * spends a seats.aero call — one per program, on Refresh or on first picking a
 * program nobody has fetched — which is the reason the cache exists at all:
 * browsing twenty-six programs live would be twenty-six calls every time this
 * tab was opened.
 *
 * Three sections, and they answer three different questions: what does THIS
 * program fly (table + map), who flies THIS PAIR (lookup), and do MY routes go
 * anywhere anyone watches (reach).
 */
export function RouteGraphPane() {
  ...
  return (
    <Stack spacing={5}>
      ...
      <Box>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          Which city pairs a program's award inventory is monitored on
        </Typography>
          ...
          <>
            ...
            {!hasGraph && fetching ? (
              ...
              <Alert severity="warning">
                This source returned <strong>no routes at all</strong>. The call succeeded — seats.aero
                answers <code>200 []</code> for a name it does not recognise, so this almost
                certainly is not a real source.
              </Alert>
            ) : (
              <>
                <Stack
