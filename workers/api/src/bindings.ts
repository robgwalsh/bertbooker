 ...
 *  source — seats.aero. No Browser Rendering binding, no Workers AI, no relay
 *  URL/token, no ENABLED_PROVIDERS: all of those existed to run *carrier sources*
 *  inside the request path. Those sources are gone; what remains that cannot
 *  run here runs on a local
 *  machine and is staying there. */
export interface Env {
   ...
   *  APP_PASSWORD`; locally, a line in `workers/api/.dev.vars`). It is compared
   ...
   *  a line in `workers/api/.dev.vars`). The input keying material for the
  ...
  /** Shared secret the local source runner presents when opening a run
   *  (`wrangler secret put INGEST_TOKEN`). Unset => no check, which is the
   *  local-dev posture. */
  INGEST_TOKEN?: string;
  /** seats.aero Partner API key (`wrangler secret put SEATS_AERO_API_KEY`;
   *  locally, a line in `workers/api/.dev.vars`). Unset => the search endpoint
   ...
   *  `workers/api/.dev.vars`). Unset => sweeps still run and still ingest, but
...
export interface Vars {
   ...
   *  Absent on the local runner's token-authenticated ingest POSTs. */
