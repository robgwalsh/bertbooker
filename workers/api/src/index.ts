...
import { ALL_ALERT_TYPES } from "@bertbooker/core";
import {
  FIND_COLUMNS,
  ROUTE_FINDS_MATCH,
  ROUTE_FINDS_SEATS,
...
//
// A source whose posture against a datacenter IP is unmeasured declares
// `runtime: "local"` and runs in `packages/local-sources` instead, POSTing to
// `/api/ingest/*`. PointsYeah is the one such source today. docs/SOURCES.md.
//
// Something DOES now run on a schedule: the alerts cron (`alerts/sweep.ts`, and
// the `scheduled` handler on the default export below) re-searches routes marked
...
app.use(
  ...
  cors({
    origin: corsOrigin,
    credentials: true,
    allowHeaders: ["Content-Type", "X-Ingest-Token"],
  }),
);
...
// SPA's JSON POSTs and the local runner's ingest POSTs pass untouched, and the
// gap it closes is a form on someone else's page aimed at this API.
...
// Everything below requires the shared password — or, for the local runner's
// ingest POSTs, the INGEST_TOKEN it already carries.
app.use("/api/*", gate);

// ...and then an identity.
app.use("/api/*", identity);

// Ingest for sources that can only run on your machine (docs/SOURCES.md).
// Mounted after `identity` so its handlers can read `userEmail`; it adds a
// shared-secret check on top.
app.route("/", ingest);
 ...
 * originates. So a tracked route is something the server can execute — against
 * every source declaring `runtime: "worker"`, which today is that one.
 */

// ---- The general query over stored finds ----
// Paged, filtered, sorted access to every stored find, with
// the freshness of each row (`last_checked_at`) carried alongside so an old find
// reads as old rather than as current.
//
// **No SPA caller, and no test.** This backed a general database browser,
// which was removed as a worse duplicate of the dashboard's own reader. Kept
// because it is the only view of the whole table that is not scoped to a tracked
// route, and it costs nothing idle. But note what "no caller" means for a route
// that shares `findsCte` with the dashboard: nothing here would notice this
// half breaking. Anything that wants it back should arrive with a test.
app.get("/api/finds", async (c) => {
  const q = buildFindsQuery((k) => c.req.query(k));

  const rows = await c.env.DB.batch([
    c.env.DB.prepare(
      `${q.cte}
       SELECT ${FIND_COLUMNS} FROM finds f
       ${q.where.length ? `WHERE ${q.where.join(" AND ")}` : ""}
       ORDER BY ${q.orderBy} LIMIT ? OFFSET ?`,
    ).bind(...q.binds, q.limit, q.offset),
    c.env.DB.prepare(
      `${q.cte}
       SELECT COUNT(*) AS total FROM finds f
       ${q.where.length ? `WHERE ${q.where.join(" AND ")}` : ""}`,
    ).bind(...q.binds),
  ]);

  return c.json({
    finds: rows[0]?.results ?? [],
    total: Number((rows[1]?.results?.[0] as { total?: number } | undefined)?.total ?? 0),
    limit: q.limit,
    offset: q.offset,
  });
});

// ---- Which sources have ever written data (powers the browser's filter) ----
app.get("/api/finds/sources", async (c) => {
  const { results } = await c.env.DB.prepare(
    `SELECT source, COUNT(*) AS finds, MAX(captured_at) AS last_write
       FROM availability_snapshots GROUP BY source ORDER BY source`,
  ).all();
  return c.json(results);
});
