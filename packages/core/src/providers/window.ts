...
//
// Extracted from pointsyeah.ts, which was the first (and for a while only)
// consumer. Deliberately NOT re-exported from there: providers/index.ts does
// `export *` from every provider module, and the same name exported by two
// modules silently disappears from the barrel.
