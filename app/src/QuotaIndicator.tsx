...
const METERED_SOURCE_LABEL: Record<string, string> = {
  seatsaero: "seats.aero Partner API",
};
 ...
 *
 * It must be byte-identical to `SEATSAERO_SOURCE_ID` in
 * `shared/src/providers/seatsaero.ts`, because that is the literal the Worker
 * writes into `source_quota.source`. It was `api:seatsaero` here for a long
 * time after a migration re-keyed the database to `seatsaero`, and the failure
 * was silent in exactly the way an id mismatch always is: every lookup matched
 * nothing, `quotaLeft` was permanently `undefined`, and the chip fell back to
 * rendering the raw id instead of a number. There is no shared constant to
 * import — the SPA mirrors the Worker's wire types by hand (see `api.ts`) — so
 * this is the seam, and it is spelled out rather than left to be rediscovered.
 */
export const PRIMARY_METERED_SOURCE = "seatsaero";
