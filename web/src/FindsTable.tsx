...
import { FindCost, FindProvenance } from "./findCells";
...
// The one table that renders stored award finds. **One caller now:** the Routes
// page (a route's current finds, route implied by its heading). The second was
// a general database browser — many routes at once, which is what the route
// and provenance columns below were for — and it is gone, being a worse second
// answer to a question the Routes page already answers off the same `findsCte`.
//
// Those options are kept rather than deleted with it: they are a handful of
// optional columns, they describe things the table genuinely can show, and a
// second multi-route caller would need exactly them back. Treat a `false` in
// their doc comments as "no caller currently asks", not as dead weight.
//
...
/** Optional columns. The two additive ones default OFF, so the Routes page keeps
 *  the table it had; `showMap` is the exception and defaults ON, so every
 *  existing caller keeps the table IT had. Adding a column and removing one
 *  cannot share a default — which is still the rule to follow when adding the
 *  next one, even though the caller that made the distinction bite has gone. */
export interface FindsTableOptions {
  /** Origin → destination. Needed the moment the table spans more than one, so
   *  no current caller sets it: the Routes page's heading names the route. */
  showRoute?: boolean;
  /**
   * Draw the routing as geography beside each itinerary. **Defaults on.**
   *
   * A user preference on the Routes page (`web/src/preferences.ts`), passed in
   ...
   */
  showMap?: boolean;
  /** Which source wrote the row, and when anybody last checked that slot.
   *  "Never checked" and "no award space" are different answers. No current
   *  caller: this was the Database pane's column. */
...
function FindRow({
  ...
  return (
    <TableRow hover>
      {opts.showRoute && (
        <TableCell sx={{ whiteSpace: "nowrap", fontWeight: 700, letterSpacing: 0.3 }}>
    ...
    <>
      ...
      <TableContainer>
        <Table size="small">
          <TableHead>
            <TableRow>
              {opts.showRoute && <TableCell>Route</TableCell>}
              ...
              {opts.showProvenance && <TableCell>Source / checked</TableCell>}
            </TableRow>
          </TableHead>
          <TableBody>
            {rows.map((f, i) => (
