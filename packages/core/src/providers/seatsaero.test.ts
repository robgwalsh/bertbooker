...
import { PROGRAM_SEEDS, currenciesForProgram } from "../data/programs.js";
...
describe("parseQuotaHeaders", () => {
  it("reads remaining and limit", () => {
    ...
    expect(parseQuotaHeaders(h, "api:seatsaero", 1234)).toEqual({
      source: "api:seatsaero",
      remaining: 812,
      limit: 1000,
      observedAt: 1234,
    });
  ...
  it("leaves limit undefined when the vendor does not state one", () => {
    ...
    expect(parseQuotaHeaders(h, "api:seatsaero", 1)).toEqual({
      source: "api:seatsaero",
      remaining: 0,
      limit: undefined,
      observedAt: 1,
  ...
  it("returns undefined rather than guessing when the header is missing", () => {
    expect(parseQuotaHeaders(new Headers(), "api:seatsaero", 1)).toBeUndefined();
    expect(
      parseQuotaHeaders(new Headers({ "x-ratelimit-remaining": "n/a" }), "api:seatsaero", 1),
    ).toBeUndefined();
  });
});
