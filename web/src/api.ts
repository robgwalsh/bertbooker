 ...
 *  `ALL_ALERT_TYPES` in packages/core/src/alerts/select.ts. */
export const ALERT_TYPES = ["new", "price_drop", "more_seats", "gone"] as const;
export type AlertType = (typeof ALERT_TYPES)[number];

/** workers/api/src/alerts/routes.ts — GET /api/alerts/schedule. */
...
/**
 * workers/api/src/alerts/sweep.ts — `TickResult`, the whole of what one tick
 * decided.
 *
 ...
 * reverse). Mirrors `RouteBody` in `workers/api/src/index.ts`.
 */
...
// packages/core/src/data/airlines.ts — a carrier plus the program codes whose
...
// Hand-mirrored from the worker. The web app has no @bertbooker/core dependency
// (core references D1Database at module scope and fights a DOM tsconfig), so
// each type below names its source file. Keep them in sync by hand until core
// grows a DOM-safe `wire` entrypoint.

// packages/core/src/ingest/types.ts. The distinction that carries the whole
...
// One row per gather, whoever asked for it: `search` and `alert` are written by
// the Worker, `local` by `npm run gather`.
...
// packages/core/src/diff.ts — stored on search_runs.changes_json, display only.
...
// workers/api/src/search.ts — one NDJSON frame from
// POST /api/tracked-routes/:id/search.
//
// This comes from the WORKER, not from `npm run gather`: the seats.aero Partner
// API is a keyed vendor API that Cloudflare's IPs can reach, so searching a
// tracked route needs nothing running on this machine. The unit of progress is a
// 90-day date chunk, and each chunk is already applied to D1 by the time its
// `chunk_done` frame lands.
/** One HTTP call to seats.aero. Mirrors `SeatsAeroCall` in
 *  `packages/core/src/providers/seatsaero.ts`.
 *
 ...
 *  in workers/api/src/enrich.ts, which is the authority. Held here only so the
 ...
 *  workers/api/src/enrich.ts. One call covers every cabin of the availability,
...
/** Mirrors `EnrichEvent` in workers/api/src/enrich.ts. */
...
// One flown leg, as stored in segments_json (mirrors @bertbooker/core Segment).
...
const SEARCH_ERRORS: Record<string, string> = {
  no_seats_aero_key:
    "seats.aero API key not configured — set SEATS_AERO_API_KEY (workers/api/.dev.vars locally, or a Worker secret).",
...
// The password gate
// ---------------------------------------------------------------------------
// Mirrors `workers/api/src/gate.ts`, by hand, like every other wire type in this
