 ...
 * graph. `pages/Routes.tsx` imports them back; this is a move, not a copy.
 *
 * The date helpers mirror `addDaysISO` / `daysBetween` in
 * `packages/core/src/providers/window.ts`. Copied rather than imported for the
 * same reason `web/src/api.ts` hand-mirrors every wire type: the SPA has no
 * `@bertbooker/core` dependency, because core references `D1Database` at module
 * scope and fights a DOM tsconfig.
 */
