 ...
 * The Alerts section.
 *
 * **This is the feature's safety mechanism, not its status board.** The app
 ...
 * same silence as one that ran and found nothing. This section is the answer:
 * every skip and every reason one route is not being swept is on it, by name.
 *
 * **So the ordering here is deliberate — problems first, cadence second, routes
 * third.** Anything that would make the mail stop is above the fold, and that
 * ordering is why this is a composition of named sections rather than one long
 * body: the sequence is the design, and it should be legible in one screen of
 * code.
 *
 ...
 * whole `TickResult`, because a surface whose entire job is making silent
 * failure visible must not grow a button that fails silently.
 */
  ...
  return (
    <Box>
      <Stack direction="row" spacing={2} sx={{ alignItems: "center", justifyContent: "space-between" }}>
        <SectionHeader
          title="Alerts"
          icon={<NotificationsActiveRoundedIcon sx={{ color: "secondary.main" }} />}
        />
        {/* mb matches SectionHeader's own, so `alignItems: center` lines the
            button up with the title rather than with the gap under it. */}
        {s?.manualTick && (
